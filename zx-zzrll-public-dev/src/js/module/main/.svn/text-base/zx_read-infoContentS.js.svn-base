$(document).ready(function() {
	...
	var aefS = sessionStorage.getItem("aefS");//获取从哪个页面返回的标识 资料页面为true，附件上传页面为false
	flag=true;//资料页面为true

	//点击资料页面的返回键设置标识为true
	$("#zx_read-infoContentS .mui-pull-left").click(function(){
    	...
    	//返回申请人或者审批不通过
			...
			$('.check-confirm').click(function() {
				...
				if ($('.check-confirm span').hasClass('read')) {
					sessionStorage.setItem("infoReadStateS", "1");
				} else {
